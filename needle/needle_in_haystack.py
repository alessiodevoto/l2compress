...
import glob
import json
from transformers import LlamaForCausalLM, AutoModelForCausalLM, AutoTokenizer, AutoConfig
...
#  transformers==4.35.2
def reset_rope(model: LlamaForCausalLM, model_max_train_len, scaling_factor):
...
class LLMNeedleHaystackTester:
    ...
    def __init__(self,
        ...
        self.adjust_kv_strategy = adjust_kv_strategy
        ...
        if (self.model_provider not in ["OpenAI", "Anthropic"]):
            ...
            if "llama-2-7b-80k" in model_name:
                ...
                model = AutoModelForCausalLM.from_pretrained(model_name, config=config, use_flash_attention_2=True,
                                                             torch_dtype=torch.float16, device_map="auto")
                model.resize_token_embeddings(32001)

                tokenizer = AutoTokenizer.from_pretrained(
                    model_name, model_max_length=context_size if context_size > orig_ctx_len else orig_ctx_len,
    ...
    def generate_prompt(self, context):
        ...
        if (self.model_provider not in ["OpenAI", "Anthropic"]):
            # todo: check the correctness of the Chat Template
    ...
    def evaluate_and_log(self, context_length, depth_percent):
        ...
        if (self.model_provider in ["OpenAI", "Anthropic"]):
            ...
            if self.snapkv:
                ...
                generation_config = self.model_to_test.generation_config.to_dict()
                response = generate_with_context(
                    self.model_to_test,
                    self.enc,
                    input_ids,
                    self.adjust_kv_strategy,
                    max_new_tokens=50,
        ...
        self.save_logs(results, context_length, context, depth_percent)

    def save_logs(self, results, context_length, context, depth_percent):
        ...
        if self.save_contexts:
            ...
            context_dir = os.path.join(log_dir, "contexts")
            ...
            if not os.path.exists(os.path.join(context_dir, f'{self.model_version}')):
                os.makedirs(os.path.join(context_dir, f'{self.model_version}'))

            # with open(f'contexts/{self.model_version}/{context_file_location}_context.txt', 'w') as f:
            #     f.write(context)

            with open(os.path.join(context_dir, f'{self.model_version}/{context_file_location}_context.txt'), 'w') as f:
                f.write(context)
        ...
        if self.save_results:
            ...
            if not os.path.exists(os.path.join(results_dir, f'{self.model_version}')):
                os.makedirs(os.path.join(results_dir, f'{self.model_version}'))

            # Save the result to file for retesting
            # p = f'results/{self.model_version}/{context_file_location}_results.json'
            p = os.path.join(results_dir, f'{self.model_version}/{context_file_location}_results.json')
    ...
    def result_exists(self, context_length, depth_percent):
        ...
        results_dir = os.path.join(results_dir, self.model_version)
    ...
    def encode_text_to_tokens(self, text, add_special_tokens=True):
        """
        [5-31 yuzhaouoe]  code
        Args:
            text:
        ...
        if self.model_provider in ["OpenAI", "LLaMA", "Mistral", "GLM"]:
            return self.enc.encode(text, add_special_tokens=add_special_tokens)
        elif self.model_provider == "Anthropic":
    ...
    def insert_needle(self, context, depth_percent, context_length):
        tokens_needle = self.encode_text_to_tokens(self.needle)  # one more bos
        tokens_context = self.encode_text_to_tokens(context)  # one more bos (2 bos)
    ...
    def read_context_files(self):
        ...
        while self.get_context_length_in_tokens(context) < max_context_length:
            ...
            assert len(file_list) > 0, "No text files"
